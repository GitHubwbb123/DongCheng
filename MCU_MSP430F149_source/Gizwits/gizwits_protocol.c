...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLED5)
    {
        info->event[info->num] = EVENT_LED5;
        info->num++;
        dataPoints->valueLED5 = gizStandardDecompressionValue(LED5_BYTEOFFSET,LED5_BITOFFSET,LED5_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED6)
    {
        info->event[info->num] = EVENT_LED6;
        info->num++;
        dataPoints->valueLED6 = gizStandardDecompressionValue(LED6_BYTEOFFSET,LED6_BITOFFSET,LED6_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED7)
    {
        info->event[info->num] = EVENT_LED7;
        info->num++;
        dataPoints->valueLED7 = gizStandardDecompressionValue(LED7_BYTEOFFSET,LED7_BITOFFSET,LED7_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagLED8)
    {
        info->event[info->num] = EVENT_LED8;
        info->num++;
        dataPoints->valueLED8 = gizStandardDecompressionValue(LED8_BYTEOFFSET,LED8_BITOFFSET,LED8_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueLED5 != cur->valueLED5)
    {
        GIZWITS_LOG("valueLED5 Changed\n");
        ret = 1;
    }
    if(last->valueLED6 != cur->valueLED6)
    {
        GIZWITS_LOG("valueLED6 Changed\n");
        ret = 1;
    }
    if(last->valueLED7 != cur->valueLED7)
    {
        GIZWITS_LOG("valueLED7 Changed\n");
        ret = 1;
    }
    if(last->valueLED8 != cur->valueLED8)
    {
        GIZWITS_LOG("valueLED8 Changed\n");
        ret = 1;
    }
    if(last->valueIN1 != cur->valueIN1)
    {
        GIZWITS_LOG("valueIN1 Changed\n");
        ret = 1;
    }
    if(last->valueIN2 != cur->valueIN2)
    {
        GIZWITS_LOG("valueIN2 Changed\n");
        ret = 1;
    }
    if(last->valueIN3 != cur->valueIN3)
    {
        GIZWITS_LOG("valueIN3 Changed\n");
        ret = 1;
    }
    if(last->valueIN4 != cur->valueIN4)
    {
        GIZWITS_LOG("valueIN4 Changed\n");
        ret = 1;
    }

    if(last->valueTEM1 != cur->valueTEM1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTEM1 Changed\n");
            ret = 1;
        }
    }
    if(last->valueTEM2 != cur->valueTEM2)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTEM2 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(LED5_BYTEOFFSET,LED5_BITOFFSET,LED5_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED5);
    gizStandardCompressValue(LED6_BYTEOFFSET,LED6_BITOFFSET,LED6_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED6);
    gizStandardCompressValue(LED7_BYTEOFFSET,LED7_BITOFFSET,LED7_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED7);
    gizStandardCompressValue(LED8_BYTEOFFSET,LED8_BITOFFSET,LED8_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLED8);
    gizStandardCompressValue(IN1_BYTEOFFSET,IN1_BITOFFSET,IN1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueIN1);
    gizStandardCompressValue(IN2_BYTEOFFSET,IN2_BITOFFSET,IN2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueIN2);
    gizStandardCompressValue(IN3_BYTEOFFSET,IN3_BITOFFSET,IN3_LEN,(uint8_t *)devStatusPtr,dataPoints->valueIN3);
    gizStandardCompressValue(IN4_BYTEOFFSET,IN4_BITOFFSET,IN4_LEN,(uint8_t *)devStatusPtr,dataPoints->valueIN4);
    ...
    devStatusPtr->valueTEM1 = exchangeBytes(gizY2XFloat(TEM1_RATIO,  TEM1_ADDITION, dataPoints->valueTEM1));     
    devStatusPtr->valueTEM2 = exchangeBytes(gizY2XFloat(TEM2_RATIO,  TEM2_ADDITION, dataPoints->valueTEM2));     
